...
const MediumQuiz = () => {
  ...
  const loadQuizHistory = async () => {
    try {
      const historyJson = await AsyncStorage.getItem('@VT_QUIZ_HISTORY_medium');
      if (historyJson) {
  ...
  const saveQuizAttempt = async (score: number, total: number, points: number) => {
    try {
      ...
      await AsyncStorage.setItem('@VT_QUIZ_HISTORY_medium', JSON.stringify(history));
      setQuizHistory(history);
    } catch (error) {
  ...
  const initializeQuiz = async () => {
    try {
      const words = await getWordsByDifficulty('medium');
  ...
  const handleSubmitAnswer = async () => {
    ...
    if (isCorrect) {
      ...
      if (!answeredWords.has(currentQuestion.word.id)) {
        const points = 15; // Medium quiz: 15 points per correct answer
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <Text style={styles.headerTitle}>Medium Quiz</Text>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
        ...
        <View style={styles.questionCard}>
          <View style={styles.questionHeader}>
            <Ionicons name="help-circle" size={28} color="#FF9800" />
        ...
        {showFeedback && (
          <View style={selectedAnswer === currentQuestion.correctAnswer ? styles.feedbackCorrect : styles.feedbackIncorrect}>
            <Ionicons 
              ...
              size={24} 
              color={selectedAnswer === currentQuestion.correctAnswer ? "#FF9800" : "#F44336"} 
            />
