...
const HardQuiz = () => {
  ...
  const loadQuizHistory = async () => {
    try {
      const historyJson = await AsyncStorage.getItem('@VT_QUIZ_HISTORY_hard');
      if (historyJson) {
  ...
  const saveQuizAttempt = async (score: number, total: number, points: number) => {
    try {
      ...
      await AsyncStorage.setItem('@VT_QUIZ_HISTORY_hard', JSON.stringify(history));
      setQuizHistory(history);
    } catch (error) {
  ...
  const initializeQuiz = async () => {
    try {
      const words = await getWordsByDifficulty('hard');
  ...
  const handleSubmitAnswer = async () => {
    ...
    if (isCorrect) {
      ...
      if (!answeredWords.has(currentQuestion.word.id)) {
        const points = 20; // Hard quiz: 20 points per correct answer
  ...
  const finishQuiz = async () => {
    ...
    try {
      await AsyncStorage.setItem('@VT_LAST_ACTIVITY', `Completed Hard Quiz (Attempt #${attemptNumber}): ${correctAnswers}/${questions.length} correct`);
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <Text style={styles.headerTitle}>Hard Quiz</Text>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
        ...
        <View style={styles.questionCard}>
          <View style={styles.questionHeader}>
            <Ionicons name="help-circle" size={28} color="#F44336" />
        ...
        {showFeedback && (
          <View style={selectedAnswer === currentQuestion.correctAnswer ? styles.feedbackCorrect : styles.feedbackIncorrect}>
            <Ionicons 
              ...
              size={24} 
              color={selectedAnswer === currentQuestion.correctAnswer ? "#F44336" : "#F44336"} 
            />
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.summaryCard}>
            <Ionicons name="trophy" size={64} color="#F44336" />
